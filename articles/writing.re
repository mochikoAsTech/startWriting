...
この章では技術書やドキュメントを通して「文章でわかりやすくつたえる」際に、こんなところに気をつけるとよりわかりやすくなりますよ、というお話をします。
...
自分の理解が足りないのに人にわかりやすく説明する文章は書けません。例を書いてみることで自分の理解度を試してみましょう。
...
技術を文章でわかりやすくつたえよう！と思ったとき、やってしまいがちな失敗に「一度に多くのことをつたえすぎる」というものがあります。
...
たとえばリクエストとレスポンスの流れについて説明しようとすると、「ブラウザでwww.example.comを開くと、名前解決が行われてウェブサーバの203.0.113.222というIPアドレスが返ってきます」というようにドメインやIPアドレスの具体例を書きたくなることがあります。このような例を書くときは例示用のドメインやIPアドレスを使いましょう。

実はインターネットでは「例示やテストで使っていいドメインやIPアドレス」というものが定められています。@<fn>{rfc}例として記載するURLやメールアドレス、IPアドレスなどは次のものをつかいましょう。
...
自分の持ち物でないドメインやIPアドレスを勝手に例示で使うことはトラブルの元になります。@<fn>{trouble}必ず例示用のドメインやIPアドレスを使いましょう。

//footnote[trouble][どんなトラブルになるのか？は「DNSをはじめよう」のP110「＜トラブル＞ test@test.co.jp を使って情報漏洩」をご覧ください。]
...
なんということでしょう！「簡潔にしようと心がけている」と言っている割に文章が冗長です。たとえばこの文章は前後を入れ替えると簡潔でわかりやすくなります。
